namespace Log4netTest
{
	partial class Log4netForm
	{
		/// <summary>
		/// 必需的设计器变量。
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary>
		/// 清理所有正在使用的资源。
		/// </summary>
		/// <param name="disposing">如果应释放托管资源，为 true；否则为 false。</param>
		protected override void Dispose(bool disposing)
		{
			if (disposing && (components != null))
			{
				components.Dispose();
			}
			base.Dispose(disposing);
		}

		#region Windows 窗体设计器生成的代码

		/// <summary>
		/// 设计器支持所需的方法 - 不要修改
		/// 使用代码编辑器修改此方法的内容。
		/// </summary>
		private void InitializeComponent()
		{
			this.btnExportMsg = new System.Windows.Forms.Button();
			this.SuspendLayout();
			// 
			// btnExportMsg
			// 
			this.btnExportMsg.Location = new System.Drawing.Point(85, 52);
			this.btnExportMsg.Name = "btnExportMsg";
			this.btnExportMsg.Size = new System.Drawing.Size(113, 37);
			this.btnExportMsg.TabIndex = 0;
			this.btnExportMsg.Text = "输出信息";
			this.btnExportMsg.UseVisualStyleBackColor = true;
			this.btnExportMsg.Click += new System.EventHandler(this.btnExportMsg_Click);
			// 
			// Log4netForm
			// 
			this.AutoScaleDimensions = new System.Drawing.SizeF(6F, 12F);
			this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			this.ClientSize = new System.Drawing.Size(447, 244);
			this.Controls.Add(this.btnExportMsg);
			this.Name = "Log4netForm";
			this.Text = "Log4netForm";
			this.ResumeLayout(false);

		}

		#endregion

		private System.Windows.Forms.Button btnExportMsg;
	}
}

